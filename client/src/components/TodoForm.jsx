...
export default function TodoForm({ title, description, setTitle, setDescription, onCreate }) {
  ...
  const showTitleError = (touchedTitle || submitted) && !!titleError
  const showDescError = (touchedDesc || submitted) && !!descError
  const canSubmit = !titleError && !descError

  const handleSubmit = (e) => {
    e.preventDefault()
    setSubmitted(true)
    if (canSubmit) onCreate(e)
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="card p-4 md:p-6 mt-4">
      ...
      <div className="mt-4 flex justify-end">
        <button type="submit" className={`btn-solid ${!canSubmit ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!canSubmit}>
